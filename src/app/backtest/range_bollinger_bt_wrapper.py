...
from src.app.strategies.range_bollinger import RangeBollingerStrategy
from market_data_mock import MarketDataMock
from order_translator import OrderTranslator

class RangeBollingerBTWrapper(bt.Strategy):
    params = dict(
        capital=100000,
        max_stocks=5,
        stop_loss=0.04,    # Trailing stop à 4%
        take_profit=0.07,  # Take profit à 7%
    ...
    def __init__(self, start_date, end_date, dataframes=None):
        ...
        # Ta stratégie métier
        self.strategy = RangeBollingerStrategy(
