...
from src.app.strategies.range_vap import RangeVapStrategy
from market_data_mock import MarketDataMock
from order_translator import OrderTranslator

class RangeVapBTWrapper(bt.Strategy):
    params = dict(
        capital=100000,
        max_stocks=5,
        stop_loss=0.04,    # Trailing stop à 4%
        take_profit=0.06,  # Take profit à 6% (retour vers le POC)
    ...
    def __init__(self, start_date, end_date, dataframes=None):
        ...
        # Ta stratégie métier
        self.strategy = RangeVapStrategy(
